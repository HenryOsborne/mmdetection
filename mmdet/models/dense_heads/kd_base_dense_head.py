...
class LDBaseDenseHead(BaseModule, metaclass=ABCMeta):
    """Base class for DenseHeads."""

    def __init__(self, init_cfg=None):
        super(LDBaseDenseHead, self).__init__(init_cfg)

    def init_weights(self):
        super(LDBaseDenseHead, self).init_weights()
    ...
    def get_bboxes(
        ...
        # =============================================================
        if isinstance(self.prior_generator, list):
            mlvl_priors = []
            for i in range(len(self.prior_generator)):
                prior = self.prior_generator[i].grid_priors(
                    [featmap_sizes[i]],
                    dtype=cls_scores[0].dtype,
                    device=cls_scores[0].device)
                mlvl_priors.append(prior[0])
        else:
    ...
    def forward_train(
        ...
        """
        Args:
            out_teacher: Teacher Model Output.
        ...
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas, out_teacher)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, out_teacher)
