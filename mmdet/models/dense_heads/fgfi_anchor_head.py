...
class FGFI_AnchorHead(FGFI_BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(
        ...
        super(FGFI_AnchorHead, self).__init__(init_cfg)
        ...
        if self.train_cfg:
            ...
            if hasattr(self.train_cfg, 'sampler') and self.train_cfg.sampler.type.split(
                    '.')[-1] != 'PseudoSampler':
                self.sampling = True
        ...
        self.FGFI_iou_calculator = build_iou_calculator(dict(type='BboxOverlaps2D'))
    ...
    def _init_layers(self):
        ...
        self.conv_reg = nn.Conv2d(
            self.in_channels, self.num_base_priors * 4,
            1)
    ...
    def loss(
        ...
        gt_bboxes = gt_bboxes[0].cpu()
        for anchors in anchor_list[0]:
            anchors = anchors.cpu()
            num_base_priors = self.num_base_priors
            h = int(math.sqrt(len(anchors) / num_base_priors))
            w = h

            IOU_map = self.FGFI_iou_calculator(gt_bboxes, anchors)
            IOU_map = IOU_map.view(h, w, num_base_priors, len(gt_bboxes))

            max_iou, _ = torch.max(IOU_map.view(h * w * num_base_priors, len(gt_bboxes)), dim=0)
            mask_per_im = torch.zeros([h, w], dtype=torch.int64).cuda()
            for k in range(len(gt_bboxes)):
                if torch.sum(gt_bboxes[k]) == 0.:
                    break
        ...
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox), mask_batch
