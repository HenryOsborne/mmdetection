...
class FGFI_BaseDenseHead(BaseModule, metaclass=ABCMeta):
    """Base class for DenseHeads."""

    def __init__(self, init_cfg=None):
        super(FGFI_BaseDenseHead, self).__init__(init_cfg)

    def init_weights(self):
        super(FGFI_BaseDenseHead, self).init_weights()
    ...
    def forward_train(
        ...
        losses, mask_batch = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore)
        if proposal_cfg is None:
            return losses
        else:
            proposal_list = self.get_bboxes(
                *outs, img_metas=img_metas, cfg=proposal_cfg)
            return losses, proposal_list, mask_batch
