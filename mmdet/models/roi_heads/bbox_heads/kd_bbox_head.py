...
class LDBBoxHead(BaseModule):
    ...
    def __init__(
            ...
            loss_ld=dict(
                type='TwoStageKnowledgeDistillationKLDivLoss', loss_weight=0.25, T=10),
            loss_kd=dict(
                type='KnowledgeDistillationCrossEntropyLoss',
                use_sigmoid=False,
                loss_weight=0.25,
                T=0.5),
            init_cfg=None):
        super(LDBBoxHead, self).__init__(init_cfg)
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes, pos_gt_labels, cfg):
        ...
        Returns:
            ...
            in a single image. Containing the following Tensors:
                ...
                - bbox_targets(Tensor):Regression target for all proposals, has shape (num_proposals, 4),
    ...
    def loss(
        ...
        if bbox_pred is not None:
            ...
            # KD
            if cls_score is not None:
                avg_factor = max(torch.sum(soft_labels_weights > 0).float().item(), 1.)
                if cls_score.numel() > 0:
            ...
            # LD
            if bbox_pred is not None:
                soft_pos_inds = (soft_labels >= 0) & (soft_labels < bg_class_ind)
                if soft_pos_inds.any():
                    if self.reg_decoded_bbox:
                        bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)

                    if self.reg_class_agnostic:
                        pos_bbox_pred = bbox_pred.view(bbox_pred.size(0), 4)[
                            soft_pos_inds.type(torch.bool)]
                    else:
                        pos_bbox_pred = bbox_pred.view(bbox_pred.size(0), -1, 4)[
                            soft_pos_inds.type(torch.bool), soft_labels[soft_pos_inds.type(torch.bool)]]
                    losses['loss_ld'] = self.loss_ld(
                        pos_bbox_pred,
                        soft_targets[soft_pos_inds.type(torch.bool)],
                        soft_targets_weights[soft_pos_inds.type(torch.bool)],
                        avg_factor=bbox_targets.size(0),
                        reduction_override=reduction_override)
                else:
                    losses['loss_ld'] = bbox_pred[soft_pos_inds].sum()
            # ==================================================================
        return losses
