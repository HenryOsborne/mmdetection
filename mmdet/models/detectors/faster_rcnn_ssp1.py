...
from mmdet.models.plugins import Heatmap1
import os
import mmcv
from mmdet.models.backbones.swin_w_ram import swin_tiny
...
class TwoStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        if backbone['type'] != 'SwinTransformerWithRAM':
            self.backbone = build_backbone(backbone)
        elif backbone['type'] == 'SwinTransformerWithRAM':
            self.backbone, hidden_dim = swin_tiny('checkpoints/swin_tiny_patch4_window7_224.pth')
            self.backbone.finetune_det(method='vidt',
                                       det_token_num=100,
                                       pos_dim=256,
                                       cross_indices=[3])
...
class FasterSSPNet1(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 neck=None,
                 pretrained=None):
        super(FasterSSPNet1, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
